...
# Optional iOS override. If omitted, app.config.js derives it from
# EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID.
EXPO_PUBLIC_GOOGLE_IOS_URL_SCHEME=com.googleusercontent.apps.yyyy
```
...
For **iOS**, `app.config.js` wires the Google Sign-In config plugin only when it
can resolve an iOS URL scheme. It derives the scheme from
`EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID` (or uses `EXPO_PUBLIC_GOOGLE_IOS_URL_SCHEME`
when provided). Android ignores this value.
...
- Native app / dev client / production: `batavasa://reset-password`
- Web only: the web URL generated by Expo for `/reset-password`
