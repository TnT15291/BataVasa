...
export async function upsertPlanItem(row: PlanItemRow): Promise<void> {
  ...
  await db.runAsync(
    `INSERT INTO finance_plan_item
     (id, user_id, name, kind, amount_cents, currency, category_id, due_day, recurrence, applies_month, reminder_id, status, active, created_at, updated_at, deleted_at, synced_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(id) DO UPDATE SET
       user_id = excluded.user_id,
       name = excluded.name,
